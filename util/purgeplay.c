...
void purge(char *filename)
{
  ...
  for (;;) {
    ...
    okay = 1;
    *reason = '\0';
    printf("%20s, Gold: %12ld, Bank: %12ld.\n", player.name, (long)player.points.gold,
...
/*
    if (player.char_specials_saved.idnum == 1074338506) {
      okay = 0;
      strcpy(reason, "Bad Pfile");
    }
*/
    if (!strcasecmp(player.name, "vega")) {
      okay = 0;
      printf("Bad Pfile");
    }
	...
	else if (player.level <= LVL_HERO - 1)
	  timeout = 60;		/* Lev 11-30: 60 days */
	else if (player.level <= LVL_HERO)
