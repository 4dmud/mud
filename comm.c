...
void free_host_list(struct meta_host_data *this);
...
char * parse_prompt(CHAR_DATA *ch);
...
#if 1
    return parse_prompt(d->character);
                        ...
                        va_list args)
{
  ...
  if (t->character)
    wantsize = size = proc_color(txt, (clr(t->character, C_NRM)), sizeof(txt));
...
void save_host_list(void)
{
  ...
  while (this != NULL)
  {
    fprintf(fl, "%s %s %ld\n", this->host_ip, this->host, this->date);
    this = this->next;
  }
  fprintf(fl, "~\n");
  fclose(fl);
...
void add_ip_to_host_list(char *host_ip, char *host, time_t date)
{
  ...
  if (!host_ip || !*host_ip || !host || !*host)
    return;
  CREATE(this, struct meta_host_data, 1);
  this->next = host_list;
  snprintf(this->host_ip, HOST_LENGTH, host_ip);
  snprintf(this->host, HOST_LENGTH, host);
  this->date = date;
  host_list = this;
}
...
{
  struct meta_host_data *this;
  for (this = host_list; this; this = this->next)
  {
    if (!strcmp(this->host_ip, ip_add))
    {
      strncpy(host, this->host, HOST_LENGTH);
      *(host + HOST_LENGTH) = '\0';
      if (strcmp(this->host, ip_add))
      {
        this->date = time(0);
        save_host_list();
      }
...
struct descriptor_data *new_descriptor(socket_t s, int copyover)
{
  ...
  if (!copyover)
  {
    ...
    {
      write_to_descriptor(desc,"Sorry, 4Dimensions is full right now... \r\b"
 ...
 */
 /*
 if (t->character && PRF_FLAGGED(t->character, PRF_PAGEWRAP))
      wordwrap(i, PAGEWIDTH(t->character), sizeof(i));
 */
...
void signal_setup(void)
{
  ...
  my_signal(SIGSEGV, core_dump_on_crash);
  ...
  my_signal(SIGCLD, SIG_IGN);  /* "automagically" delete zombies */
#else
 # ifdef SIGCHLD
  my_signal(SIGCHLD, sigchld);  /* remove zombies */
# endif /* SIGCHLD */
 #endif /* SIGCLD */
}
  ...
  const char *brag[] =
    {
      ...
      "Bahaha! $N should stick to Odif's!",
...
char * parse_prompt(CHAR_DATA *ch)
{
  ...
  {
    count = snprintf(prompt + len, psize - len, "[MAIL] ");
    if (count >= 0)
      len += count;
  }
  ...
  if ((has_note(ch, NOTE_NOTE) && len < psize)
      && (PRF_FLAGGED(ch, PRF_MAIL)))
  {
    count = snprintf(prompt + len, psize - len, "[NOTE] ");
    if (count >= 0)
      len += count;
  }
  if ((has_note(ch, NOTE_NEWS) && len < psize)
      && (PRF_FLAGGED(ch, PRF_MAIL)))
  {
    count = snprintf(prompt + len, psize - len, "[NEWS] ");
    if (count >= 0)
      len += count;
  }
  if ((has_note(ch, NOTE_CHANGES) && len < psize)
      && (PRF_FLAGGED(ch, PRF_MAIL)))
  {
    count = snprintf(prompt + len, psize - len, "[CHANGES] ");
    if (count >= 0)
      len += count;
  }
  if ((has_note(ch, NOTE_IDEA) && len < psize)
      && (PRF_FLAGGED(ch, PRF_MAIL)))
  {
    count = snprintf(prompt + len, psize - len, "[IDEA] ");
    if (count >= 0)
      len += count;
  }
  ...
  {
    count = snprintf(prompt + len, psize - len, "[\x1B[1;31mKILLER\x1B[0;0m] ");
    if (count >= 0)
      len += count;
  }
  ...
  {
    count = snprintf(prompt + len, psize - len, "(i%d) ", GET_INVIS_LEV(ch));
    if (count >= 0)
      len += count;
  }
  ...
  {
    count = snprintf(prompt + len, psize - len, "BUILDWALKING ");
    if (count >= 0)
      len += count;
  }
      ...
      while (inbuf[inpos] != '\0' && inpos < MAX_PROMPT_LENGTH)
      {
        ...
        if (inbuf[inpos] == PROMPT_CHAR)
        {
          *insert_text = '\0';
          ...
          if ((strlen(out_buf) + strlen(insert_text)) < MAX_PROMPT_LENGTH)
          {
  ...
  out_buf[outpos] = '\0';
  snprintf(prompt + len, psize - len, "%s", out_buf);
#if 0
  if (!PRF_FLAGGED(ch, PRF_BATTLESPAM))
  {
    if (GET_LAST_PROMPT(ch) && !strcmp(GET_LAST_PROMPT(ch), prompt))
    {
      strcpy(prompt, "");
      return prompt;
    }
...
char *wordwrap(char *cmd, int width, size_t maxlen)
{
  ...
  */
  buf = (char *) alloca( ((len / width) * 3) + len);
