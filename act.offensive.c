...
ACMD(do_slay)
{
	...
	if (!(vict = get_char_vis(ch, arg, NULL, FIND_CHAR_ROOM)))
	    ...
	    act("Your fireball turns $N into a blazing inferno.", FALSE,
		ch, NULL, vict, TO_CHAR);
	    act("$n releases a searing fireball in your direction.", FALSE,
		...
		" body into tiny shards.", FALSE, ch, NULL, vict,
		TO_NOTVICT);
	} else if (!str_cmp(arg2, "demon")) {
	    act("You gesture and a slavering demon appears.  With a horrible grin,\r\n" "the foul creatre turns on $N, who screams in panic before being\r\n" "eaten alive.", FALSE, ch, NULL, vict, TO_CHAR);
	    ...
	    act("Leaping suddenly, $n sinks $s fangs into $N's throat.  As blood\r\n" "sprays and gushes to the ground, $n tosses $N's dying body away", FALSE, ch, NULL, vict, TO_NOTVICT);
