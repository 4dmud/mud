		...
		case AEDIT_CONFIRM_SAVESTRING:
			switch ( *arg )
			{
				...
				case 'Y':
					...
					d->Output ( "Action saved to memory.\r\n" );
					break;
				case 'n':
				case 'N':
		...
		case AEDIT_MAIN_MENU:
			switch ( *arg )
			{
				...
				case '1':
					...
					return;
				case '2':
					d->Output ( "Enter the minimum position the Character has to be in to activate social [0 - 8]: " );
					OLC_MODE ( d ) = AEDIT_MIN_CHAR_POS;
					return;
				case '3':
					d->Output ( "Enter the minimum position the Victim has to be in to activate social [0 - 8]: " );
