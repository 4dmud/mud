...
static variable_name_t VariableNameTable[eMSDP_MAX+1] =
{
   ...
   { eMSDP_STR_ADD,          "STR_ADD",          NUMBER_READ_ONLY },
   ...
   { eMSDP_STR_ADD_PERM,     "STR_ADD_PERM",     NUMBER_READ_ONLY },
...
protocol_t *ProtocolCreate( void )
{
   ...
   protocol_t *pProtocol = (protocol_t*)malloc(sizeof(protocol_t));
...
void MSDPSend( descriptor_t *apDescriptor, variable_t aMSDP )
{
   ...
   if ( aMSDP > eMSDP_NONE && aMSDP < eMSDP_MAX )
   {
      ...
      else /* It's an integer, not a string */
      {
         if ( pProtocol->bMSDP )
         {
            sprintf( MSDPBuffer, "%c%c%c%c%s%c%lld%c%c",
               IAC, SB, TELOPT_MSDP, MSDP_VAR,
               VariableNameTable[aMSDP].pName, MSDP_VAL,
         ...
         {
            sprintf( MSDPBuffer, "%c%c%cMSDP.%s %lld%c%c",
               IAC, SB, TELOPT_ATCP,
               VariableNameTable[aMSDP].pName,
               pProtocol->pVariables[aMSDP]->ValueInt, IAC, SE );
         }
...
void MSDPSetNumber( descriptor_t *apDescriptor, variable_t aMSDP, gold_int aValue )
{
   protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
string msdp_exits ( const Character *ch )
{
    ...
    for ( int door = 0; door < NUM_OF_DIRS; door++ )
    {
        ...
        if ( EXIT_FLAGGED ( EXIT ( ch, door ), EX_HIDDEN ) && !IS_IMM ( ch ) )
            continue;
        ...
        if ( EXIT_FLAGGED ( EXIT ( ch, door ), EX_CLOSED ) )
        {
            ...
            {
                exits += "x";
                exits.back() = toupper ( *dirs[door] );
            }
        }
        else
        {
            exits += "x";
            exits.back() = *dirs[door];
        }
    }
    return exits;
}
    ...
    for (d = descriptor_list; d; d = d->next)
    {
        ...
        if ( ch && !IS_NPC(ch) && d->connected == CON_PLAYING )
        {
            ...
            if ( pRoom )
            {
                ...
                parse_room_name ( pRoom, buf, sizeof ( buf ) );
                MSDPSetString ( d, eMSDP_ROOM_NAME, buf );
                MSDPSetString ( d, eMSDP_AREA_NAME, get_zonename ( ch ).c_str() );

                if ( pRoom->number != d->pProtocol->LastRoomVnum )
                {
                    d->pProtocol->LastRoomVnum = pRoom->number;
                    MSDPSetString( d, eMSDP_LOCATION_MAP, msdp_map(ch) );
                    string exits = msdp_exits ( ch );
                    MSDPSetString( d, eMSDP_ROOM_EXITS, exits.c_str() );
                }
            }
            else
            {
                ...
                MSDPSetString ( d, eMSDP_LOCATION_MAP, "" );
...
/******************************************************************************
 MCCP global functions.
 ******************************************************************************/

void MCCPBegin( descriptor_t *apDescriptor )
{
   if ( apDescriptor->pProtocol->bMCCP )
   {
      if ( apDescriptor->comp->state != 2 )
      {
         apDescriptor->comp->state = 2;
         apDescriptor->comp->compression = 2;
         toggle_compression( apDescriptor );
      }
   }
}
...
static void SendMSSP( descriptor_t *apDescriptor )
{
   ...
   MSSP_t MSSPTable[] =
   {
      ...
      /* Generic */
      { "CRAWL DELAY",        "-1" },
      { "HOSTNAME",           "4dimensions.org" },
      { "PORT",               "6000" },
      { "CODEBASE",           "CircleMUD" },
      { "CONTACT",            "molly(dot)4d@tele2(dot)se" },
      { "CREATED",            "1996" },
      { "ICON",               "" },
      { "IP",                 "64.62.231.178" },
      { "LANGUAGE",           "English" },
