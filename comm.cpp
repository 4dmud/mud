...
void copyover_recover(void) {
    ...
    for (;;) {
        ...
        if ((player_i = pi.LoadChar(name, d->character)) >= 0) {
...
void *process_io(void *loopstate) {
    ...
    if (loopstate == NULL) { /* we're a thread */
...
/*TODO:PROTOCOL: No longer needed
    ...
    const unsigned char do_ga[] = {
                                      IAC,DO,GA,0
                                  };
    const unsigned char do_ech[] = {
...
void make_wholist(void) {
    ...
    if (port != 6000)
        return;
    if ((fl = fopen("/var/local/4dimensions/wholist.xml", "w")) == 0) {
        if (++xml_log_trys < 4)
