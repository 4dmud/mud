...
char *parse_object ( FILE * obj_f, int nr, zone_vnum zon )
{
	...
	if ( ( retval =
	            sscanf ( line, "%d %d %d %d %d %d %d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4, t + 5, t + 6, t + 7, t + 8, t + 9, t + 10 ) ) != 11 )
	{
		if ( retval == 4 )
		{
			t[4] = 0;
		...
		{
			log ( "SYSERR: Format error in second numeric line (expecting 11 args, got %d), %s", retval, buf2 );
			exit ( 1 );
		}
	}
