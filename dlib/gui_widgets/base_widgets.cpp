...
// ----------------------------------------------------------------------------------------
    ...
    {
        if (enabled && !hidden && is_depressed_)
        {
            is_depressed_ = false;
            parent.invalidate_rectangle(rect);
            on_button_up(false);
...
// ----------------------------------------------------------------------------------------
    ...
    ) const
    {
        ...
        if (enabled && !hidden && is_depressed_ && rect.contains(lastx,lasty) == false)
        {
            ...
            // and over.
            parent.trigger_user_event((void*)this,0);
            return false;
        }
...
// ----------------------------------------------------------------------------------------
    ...
    {
        ASSERT (pos <= max_slider_pos(),
                "\tvoid scroll_bar::set_slider_pos()"
                << "\n\tpos:            " << pos 
                << "\n\tmax_slider_pos(): " << max_slider_pos() );
