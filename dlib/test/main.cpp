...
int main (int argc, char** argv)
{
    try
    {
        ...
        parser.add_option("d","Print debugging statements to file debug.txt.");
        parser.add_option("l","Set the logging level (all, trace, debug, info, warn, error, or fatal).",1);
        ...
        ofstream fout;
        if (parser.option("d").count())
        {
            logger l("test");
            if (parser.option("a").count())
        ...
        for (unsigned long i = 0; i < num; ++i)
        {
            ...
            while (testers().move_next())
            {
                ...
                for (unsigned long j = 0; j < parser.option("runall").count() + opt.count(); ++j)
                {
                    ...
                    {
                        case 0:
                            passed = test.perform_test();
                            break;
                        case 1:
                            passed = test.perform_test(opt.argument(0,j));
                            break;
                        case 2:
                            passed = test.perform_test(opt.argument(0,j), opt.argument(1,j));
                            break;
                        default:
