...
namespace  
{
    ...
    public:
        ...
        {
            int n = 0;
            n += binary_search_tree_kernel_test<binary_search_tree<int,int, 
            memory_manager_global<char,factory>::kernel_1a>::kernel_1a>    ("kernel_1a /w memory_manager_global");
            print_spinner();
            n += binary_search_tree_kernel_test<binary_search_tree<int,int, 
            memory_manager_stateless<char>::kernel_1a>::kernel_1a>    ("kernel_1a /w memory_manager_stateless");
            print_spinner();
            return (n == 0);
        }
    } a;
