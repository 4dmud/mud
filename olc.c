...
ACMD(do_olc)
{
  ...
  if (strcmp(GET_NAME(ch), "Ras")) {
    new_send_to_char(ch, "OLC is not yet complete.  Sorry.\r\n");
    return;
  }
  ...
  if ((olc_mode = search_block(mode_arg, olc_modes, FALSE)) < 0) {
    new_send_to_char(ch, "Invalid mode '%s'.\r\n%s", mode_arg, OLC_USAGE);
    return;
  }
  switch (olc_mode) {
  case OLC_SET:
  ...
  case OLC_REPEAT:
	...
	((olc_targ = GET_LAST_OLC_TARG(ch)) == NULL)) {
      new_send_to_char(ch, "No last OLC operation!\r\n");
      return;
    }
    break;
  case OLC_ROOM:
    if (isdigit(*argument)) {
      ...
      if (!is_number(arg)) {
	new_send_to_char(ch, "Invalid room vnum '%s'.\r\n", arg);
	return;
      }
      vnum = atoi(arg);
      if ((rnum = real_room(vnum)) == NULL) {
	new_send_to_char(ch, "No such room!\r\n");
	return;
      }
    } else {
      rnum = IN_ROOM(ch);
      vnum = GET_ROOM_VNUM(IN_ROOM(ch));
      new_send_to_char(ch, "(Using current room %d)\r\n", vnum);
    }

/*   if (!ROOM_FLAGGED(rnum, ROOM_OLC))
	 new_send_to_char(ch, "That room is not modifyable.\r\n");
     else
*/
    olc_targ = (void *) rnum;
    break;
  case OLC_MOB:
    argument = one_argument(argument, arg);
    if (!is_number(arg)) {
      new_send_to_char(ch, "Invalid mob vnum '%s'.\r\n", arg);
      return;
    }
    vnum = atoi(arg);
    if ((mrnum = real_mobile(vnum)) == NOBODY)
      new_send_to_char(ch, "No such mobile vnum.\r\n");
    else
      olc_targ = (void *) &(mob_proto[mrnum]);
    break;
  case OLC_OBJ:
    argument = one_argument(argument, arg);
    if (!is_number(arg)) {
      new_send_to_char(ch, "Invalid obj vnum '%s'\r\n", arg);
      return;
    }
    vnum = atoi(arg);
    if ((ornum = real_object(vnum)) == NOTHING)
      new_send_to_char(ch, "No object with vnum %d.\r\n", vnum);
    else
      olc_targ = (void *) &(obj_proto[ornum]);
    break;
  default:
    new_send_to_char(ch, "Usage: olc {.|set|show|obj|mob|room} [args]\r\n");
    return;
  }
  ...
  if (!can_modify(ch, vnum)) {
    new_send_to_char(ch, "You can't modify that.\r\n");
    return;
  }
...
void olc_interpreter(void *targ, int mode, char *arg)
{
  ...
  if ((command = search_block(command_string, olc_commands, FALSE)) < 0) {
    new_send_to_char(olc_ch, "Invalid OLC command '%s'.\r\n", command_string);
    return;
  }
  switch (mode) {
  case OLC_ROOM:
...
void olc_string(char **string, size_t maxlen, char *arg)
{
  ...
  if (!*arg) {
    new_send_to_char(olc_ch, "Enter new string (max of %d characters); use '@' on a new line when done.\r\n", (int) maxlen);
    **string = '\0';
    string_write(olc_ch->desc, string, maxlen, 0, NULL);
  } else {
    if (strlen(arg) > maxlen) {
      new_send_to_char(olc_ch, "String too long (cannot be more than %d chars).\r\n", (int) maxlen);
    } else {
      if (*string != NULL)
	free(*string);
      *string = strdup(arg);
      new_send_to_char(olc_ch, "%s", CONFIG_OK);
    }
  }
}
...
void olc_bitvector(int *bv, const char **names, char *arg)
{
  ...
  if (!*arg) {
    new_send_to_char(olc_ch, "Flag list or flag modifiers required.\r\n");
    return;
  }
  ...
  while (*arg) {
    ...
    if ((flagnum = search_block(this_name, names, TRUE)) < 0)
      new_send_to_char(olc_ch, "Unknown flag: %s\r\n", this_name);
    else {
      if (doremove)
  ...
  *bv = newbv;
  new_sprintbit(newbv, names, buf, sizeof(buf));
  new_send_to_char(olc_ch, "Flags now set to: %s\r\n", buf);
}
